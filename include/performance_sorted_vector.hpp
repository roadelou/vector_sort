...
std::chrono::microseconds performance_sorted_vector(std::unique_ptr<SortedVector<unsigned int>> &sort_factory, std::array<unsigned int, 1000> workload);
...
std::array<unsigned int, 1000> random_numbers(void) {
	...
	// random_device.
	std::for_each(numbers.begin(), numbers.end(), [](unsigned int &n) {n=std::random_device()();});
