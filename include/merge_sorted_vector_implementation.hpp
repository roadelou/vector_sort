...
template <typename Sortable> std::vector<Sortable> merge_sort(std::vector<Sortable> unsorted);

// Constructor implementation.
template <typename Sortable> MergeSortedVector<Sortable>::MergeSortedVector(void) {
	// Creating inner vector.
	this->inner = new std::vector<Sortable>();
}


template <typename Sortable> void MergeSortedVector<Sortable>::insert(Sortable element) {
...
template <typename Sortable> size_t MergeSortedVector<Sortable>::get_size(void) {
	// Calling the appropriate std::vector function.
	return this->inner->size();
}


template <typename Sortable> std::vector<Sortable> MergeSortedVector<Sortable>::get_frozen(void) {
...
template <typename Sortable> std::vector<Sortable> merge_sort(std::vector<Sortable> unsorted) {
	...
	if (size <= 1) {
		...
		// unsorted vector.
		int half = size / 2;
		std::vector<Sortable> first_vector(unsorted.begin(), std::next(unsorted.begin(), half));
		std::vector<Sortable> second_vector(std::next(unsorted.begin(), half), unsorted.end());

		// Recursively sorting the two halves of the input vector.
		std::vector<Sortable> sorted_first_vector = merge_sort<Sortable>(first_vector);
		std::vector<Sortable> sorted_second_vector = merge_sort<Sortable>(second_vector);
		...
		typename std::vector<Sortable>::iterator first_iterator = sorted_first_vector.begin();
		typename std::vector<Sortable>::iterator second_iterator = sorted_second_vector.begin();
		...
		while (first_iterator != sorted_first_vector.end()) {
			...
			// the first list.
			while (second_iterator != sorted_second_vector.end() and *first_iterator > *second_iterator) {
